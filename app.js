...
function renderRoom(room){
  ...
  if(!state.isHost && state.roomCode && !players[state.myId]){
    leaveRoomUi("El host te ha expulsado de la sala");
    return;
  }

  const myPlayer=players[state.myId];
  if(myPlayer && Number(myPlayer.coins||0)>Number(state.profile.coins||0)){
    state.profile.coins=Number(myPlayer.coins||0);
    saveProfile();
  }

  $("#hostControls").classList.toggle("hidden",!state.isHost);
  $("#gameSelector").value=room.activeGame||"guess";
  if($("#maxPlayersSelect")) $("#maxPlayersSelect").value=String(room.maxPlayers||20);
  if($("#lockRoomBtn")){
    $("#lockRoomBtn").textContent=room.locked?"🔒 Desbloquear sala":"🔓 Bloquear sala";
    $("#lockRoomBtn").classList.toggle("danger-soft",!!room.locked);
  }
  if($("#roomStatusText")){
  ...
  $("#playersList").innerHTML=entries.map(([id,p])=>`<div class="player">
      ...
      <div class="player-right">
        ...
        ${state.isHost && id!==room.hostId?`<div class="player-actions">
          <button class="mini-action coin-action" data-player="${id}" title="Dar 100 Coins">+100 🪙</button>
          <button class="mini-action kick-action" data-player="${id}" title="Expulsar jugador">✕</button>
        </div>`:""}
      </div>
    </div>`).join("") || `<div class="empty">Sin jugadores</div>`;

  if(state.isHost){
    $$(".coin-action",$("#playersList")).forEach(btn=>btn.onclick=async()=>{
      const id=btn.dataset.player;
      const current=Number((state.room.players||{})[id]?.coins||0);
      await updateNested(`players/${id}/coins`,current+100);
      toast("+100 Coins enviados");
    });
    $$(".kick-action",$("#playersList")).forEach(btn=>btn.onclick=async()=>{
      const id=btn.dataset.player;
      const player=(state.room.players||{})[id];
      if(!player)return;
      if(confirm(`¿Expulsar a ${player.name} de la sala?`)){
        await updateNested(`players/${id}`,null);
        toast(`${player.name} ha sido expulsado`);
      }
    });
  }
...
$("#maxPlayersSelect").onchange=e=>{
  if(!state.isHost)return;
  const maxPlayers=Number(e.target.value||20);
  const current=Object.values(state.room?.players||{}).filter(Boolean).length;
  if(maxPlayers<current){
    toast(`Ya hay ${current} jugadores conectados`);
    e.target.value=String(state.room.maxPlayers||20);
    return;
  }
  updateRoom({maxPlayers});
};
$("#lockRoomBtn").onclick=()=>{
  if(!state.isHost)return;
  updateRoom({locked:!state.room?.locked});
};
$("#cancelGameBtn").onclick=()=>{
  if(!state.isHost)return;
  updateRoom({status:"lobby",gameState:null});
